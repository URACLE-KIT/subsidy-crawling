    ...
    try:
        ...
        for page in range(start_page, end_page+1):
            for card_count in range(1,13):
                ...
                try:
                    cur.execute("SELECT * FROM subsidies WHERE title = %s AND description = %s", (subsidy_title, subsidy_desc))
                    existing_data = cur.fetchone()

                    if existing_data:
                        print(f"Data with title '{subsidy_title}' and description '{subsidy_desc}' already exists. Skipping insertion.")
                    else:
                        # Perform the insertion
                        sql = "INSERT INTO subsidies (application_period, application_process, application_process_url, category, description, detail_information_url, receiving_agency, support_type, telephone_inquiry, title) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"
                        ...
                        conn.commit()
                    print(f"Inserted new data with title '{subsidy_title}' and description '{subsidy_desc}'")
...
schedule.every().monday.at("00:00").do(scrape_and_insert_data)
